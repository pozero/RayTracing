...
layout(std430, set = 0, binding = 2) readonly buffer triangle_geometries_materials {
    material_t triangle_materials[];
};

void main() {
    const uint triangle_index = gl_VertexIndex / 3;
    const uint vertex_index = gl_VertexIndex % 3;
    const triangle_t triangle = triangles[triangle_index];
    const uint vertex_indices[3] = uint[3](triangle.a,
                                           triangle.b,
                                           triangle.c);
    const triangel_vertex_t vertex = triangle_vertices[vertex_indices[vertex_index]];
