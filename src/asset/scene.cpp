...
std::tuple<render_options, camera, scene> load_scene(
    ...
    for (auto const& [key, val] : mat_json.items()) {
        ...
        float const metallic = val.value<float>("/metallic"_json_pointer, 0.0f);
        ...
        std::array textures{
            ...
            val.value<std::string>("/metallic_roughness_tex"_json_pointer, ""),
        };
    ...
    for (uint32_t i = 0; i < medium_types.size(); ++i) {
        if (root_json.contains(medium_type_keys[i])) {
            ...
            for (auto const& [key, val] : med_json.items()) {
                ...
                float const anisotropic = val.at("/anisotropic"_json_pointer);
                medium const med{
                    .color = color,
                    .density = density,
                    .anisotropic = anisotropic,
